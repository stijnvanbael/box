...
const _generatedValues = [generatedInt];
...
class PostgresBox extends Box {
  ...
  Future<K> store<K>(dynamic entity) async {
    ...
    final entitySupport = registry.lookup(entity.runtimeType);
    final tableName = _snakeCase(entitySupport.name);
    var fields = entitySupport.fields
        .where((field) => !_isGenerated(entity, entitySupport, field));
    final fieldNames = fields.map((field) => _snakeCase(field));
    final fieldValues = _addEntityValues('', {}, entity);
  ...
  bool _isGenerated<T>(
    dynamic entity,
    EntitySupport<T> entitySupport,
    String field,
  ) =>
      entitySupport.isAutogenerated(field) &&
      _generatedValues.contains(entitySupport.getFieldValue(field, entity));
  ...
  Map<String, dynamic> _addEntityValues(
    ...
    for (var field in entitySupport.fields) {
      final value = entitySupport.getFieldValue(field, entity);
      if (value != null &&
          !(entitySupport.isAutogenerated(field) &&
              _generatedValues.contains(value))) {
